...
def RHSswe(u,v,p,depth,coriolis,gravity,dxs, timeCurrent, visc = 10**-3, alin=1, periodic=0):
    ''' Compute rhs of mass and mom conservation as
    ru,rv,rp=RHSswe(u,v,p,depth,coriolis,gravity,dxs, timeCurrent, alin)
    alin=0 or 1, computes the linear or nonlinear SWE
    '''
    #test periodicity at every. step.
    #NEXT STEP - KEEP TESTING WHY PERIODICITY IS BREAKING WITH LAPLACIAN!
    #THEN delete all these asserts.
    assert((u[:,-1]==u[:,1]).all())
    assert((v[:,-1]==v[:,1]).all())
    assert((p[:,-1]==p[:,1]).all())
...
# Compute water depth at z-points
    ...
    assert((hbxy[:,-1]==hbxy[:,1]).all())
    assert((hbxy[:,-2]==hbxy[:,0]).all())
...
####### end- compute total head ##########################################
    assert((rp[:,-1]==rp[:,1]).all())
    assert((rp[:,-2]==rp[:,0]).all())
...
####### end- Pressure gradients in x-y momentum equations##################
    assert((ru[:,-1]==ru[:,1]).all())
    assert((ru[:,-2]==ru[:,0]).all())
...
#experiment - in periodic case, we don't need ghost points, because we will assign boundary values with periodicity.
    u_visc = np.zeros((u.shape[0]+2,u.shape[1]+2))
    u_visc[1:-1,1:-1] = u.copy()
    u_visc[   0,1:-1] = u[ 0,:].copy()                 #Free-slip boundary, assign same vals
    u_visc[  -1,1:-1] = u[-1,:].copy()

    u_visc[:, 0] = u_visc[:,-4].copy()
    u_visc[:,-1] = u_visc[:,3].copy()

    hbx = (u_visc[:-2,1:-1] - 2*u_visc[1:-1,1:-1] + u_visc[2:,1:-1])/dxs[1]**2     \
            + (u_visc[1:-1,:-2] - 2*u_visc[1:-1,1:-1] + u_visc[1:-1,2:])/dxs[0]**2
    # hbx = np.hstack((hbx[:,-1:], hbx, hbx[:,:1]))    
# ###### Add U-Viscous Terms#################################################
...
#     if periodic:
#         u_visc[1:-1,    0] = u[:, -3].copy() #GURB to -3 and 2
...
#                 + (u_visc[1:-1,:-2] - 2*u_visc[1:-1,1:-1] + u_visc[1:-1,2:])/dxs[0]**2
    ...
    if periodic:
        # hbx[:,1] = 0
        # hbx[:,-2] = 0
        assert((hbx[:,0] == hbx[:,-2]).all())
        assert((hbx[:,1] == hbx[:,-1]).all()) #yep - already periodic if we're here
        # hbx[:,0] = hbx[:,-2].copy() #shouldn't need to do this actually - should already be the same!
        # hbx[:,-1] =hbx[:,1].copy()
...
###### Add V-Viscous Terms#################################################
    ...
    if periodic:
        v_visc[1:-1,   0] = v[:, -3].copy() #GURB to -3 and 2
        v_visc[1:-1,  -1] = v[:,2].copy()
    else:
...
###### end- Add U-Viscous Terms############################################
    ...
    if periodic:   
        ...
        #test - try to implement periodic boundaries! - might be unnecessary?
        ru[:, 0] = ru[:, -2]  # Wrap west to east
        ru[:, -1] = ru[:, 1]  # Wrap east to west

        rv[:, 0] = rv[:, -2]  # Wrap west to east
        rv[:, -1] = rv[:, 1]  # Wrap east to west

        rp[:, 0] = rp[:, -2]  # Wrap west to east
        rp[:, -1] = rp[:, 1]  # Wrap east to west

    else: #clsoe east and west boundaries if not periodic!
        ru[:,0] = ru[:,-1] = 0.0  # closed boundary west-east BC
    
#     ru += visc * u_visc_final
#     rv += visc * v_visc_final



    return ru,rv,rp

def RK3(u,v,p, depth, coriolis, gravity, dxs, dt, timeCurrent, visc=10**-3, alin=1.0, periodic=0):   
    """ Runge Kutta 3 integration """
    ru,rv,rp = RHSswe(u,v,p,depth,coriolis,gravity,dxs,timeCurrent,visc,alin,periodic)       # stage 1 tendency
    pt = p + rp*dt    # stage 1 solution
    ut = u + ru*dt    # stage 1 solution
    vt = v + rv*dt    # stage 1 solution

    ru,rv,rp = RHSswe(ut,vt,pt,depth,coriolis,gravity,dxs,timeCurrent+dt, visc, alin,periodic)  # stage 2 tendency
    pt = 0.75 * p + 0.25 * (pt + rp*dt) # stage 2 solution
    ut = 0.75 * u + 0.25 * (ut + ru*dt) # stage 2 solution
    vt = 0.75 * v + 0.25 * (vt + rv*dt) # stage 2 solution

    ru,rv,rp = RHSswe(ut,vt,pt,depth,coriolis,gravity,dxs,timeCurrent+0.5*dt, visc, alin,periodic) # stage 3 tendency
