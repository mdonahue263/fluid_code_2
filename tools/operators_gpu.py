...
def Discrete1DGrid(smin,smax,Ns):
    ...
    '''
    se,ds = cp.linspace(smin,smax,Ns+1,retstep=True) # cell edges
...
# Compute water depth at v-points
    hby = cp.zeros_like(v)                  # create array
...
###### Don't do any of this if viscosity is 0
    if visc != 0:
        ...
        if periodic:
            hbx[:,0:2] = hbx[:,-2:] = cp.zeros_like(hbx[:,0:2])
...
def initialize_vortex(xu,xv,xp,yp,xz,yz,dx,dy,grav,f0,a,L,x_center,y_center):
    ...
    rp = cp.sqrt((xp-x_center)**2 + (yp-y_center)**2)
    rz = cp.sqrt((xz-x_center)**2 + (yz-y_center)**2)
    ...
    vort_inner =(1-vort_ratio*a*cp.exp(-(rz**2)/(L**2)))**(1/2)
...
def current_vorticity(xz,u,v,dxs):
    ...
    vort_grid = cp.zeros_like(xz)

    x_vort = cp.zeros_like(xz[1:-1,1:-1])
    y_vort = x_vort.copy()
