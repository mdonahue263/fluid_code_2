...
def xop_1d(g, f, dx=1, avg=True, add=False):
    if add:
        s = 1.0
    else:
        s = 0.0
...
def RHSswe_1d(u, p, depth, gravity, dx, visc=10**-3, bdc=0, tau=0, alin=1, periodic=0):
    N = len(p) # number of cells

    h = depth + alin*p #water depth at p-points

    hbx = np.zeros_like(u)
    xop_1d(hbx, h) #need boundary help because u points averaged from P-points to interior


    if periodic: #periodic BC
        hbx[0] = hbx[-2].copy()
        hbx[-1] = hbx[1].copy()
    else: #assume u boundary the same as h boundary
        hbx[0] = h[0].copy()
        hbx[-1] = h[-1].copy()
...
###### compute total head #################
    ...
    if alin:
        xop_1d(rp, ru, dx=-2,add=True) #add u contribution of total head at P points
                                        # no boundary help needed, I think
###### end compute total head

####### Pressure gradient in x direction #######################################
    xop_1d(ru, rp, dx, avg=False) #need boundary adjustment for periodic or not!
...
####### End pressure gradient in x direction ##################################
###### Mass-flux divergence in mass equations##############################
    xop_1d(rp, U, -dx, avg=False, add=False)

    #shouldn't need boundary help here - u and HBX are already dealt with.
    if periodic:
        rp[0] = rp[-2].copy()
        rp[-1]=rp[1].copy()
...
#viscous term
    if periodic:
        ...
        u_visc = np.zeros((len(u)+2))
        u_visc[1:-1]=u.copy()
        u_visc[0] = u[0].copy()
        u_visc[-1] = u[-1].copy()

        u_visc = (u_visc[:-2] - 2*u_visc[1:-1] + u_visc[2:])/dx**2
...
def RK3_1d(u,p, depth, gravity, dx, dt, timeCurrent, visc=10**-3, bdc=0,tau=0,alin=1.0, periodic=0):   
    """ Runge Kutta 3 integration """
    ru,rp = RHSswe_1d(u,p,depth,gravity,dx,visc,bdc,tau,alin,periodic)       # stage 1 tendency
    pt = p + rp*dt    # stage 1 solution
    ut = u + ru*dt    # stage 1 solution

    ru,rp = RHSswe_1d(ut,pt,depth,gravity,dx, visc,bdc,tau, alin,periodic)  # stage 2 tendency
    pt = 0.75 * p + 0.25 * (pt + rp*dt) # stage 2 solution
    ut = 0.75 * u + 0.25 * (ut + ru*dt) # stage 2 solution

    ru,rp = RHSswe_1d(ut,pt,depth,gravity,dx, visc,bdc,tau, alin,periodic) # stage 3 tendency
